...
##### Page "Carte des polluants"
    ...
    if selected_pollutant:
        ...
        st.markdown(f"### À propos du polluant sélectionné : {selected_pollutant}")
        st.markdown(f"- **Limite normale recommandée** : {pollutant_limits.get(selected_pollutant, 'Non disponible')}")
        st.markdown(f"- **Conseil** : {pollutant_tips.get(selected_pollutant, 'Aucune recommandation disponible.')}")
        st.markdown(f"[🌐 En savoir plus sur {selected_pollutant}](https://fr.wikipedia.org/wiki/{selected_pollutant})")
...
##### Page Infos touristiques de la ville"
    ...
    countries = sorted(data['Country Label'].unique())
    default_country = st.session_state.get("selected_country", countries[0])
    selected_country = st.selectbox("Sélectionnez un pays :", options=countries, index=countries.index(default_country))

    # Filtrer les villes en fonction du pays sélectionné
    filtered_data = data[data['Country Label'] == selected_country]
    cities = sorted(filtered_data['City'].dropna().unique())
    default_city = st.session_state.get("selected_city", cities[0] if cities else None)
    selected_city = st.selectbox("Sélectionnez une ville :", options=cities, index=cities.index(default_city) if default_city in cities else 0)
    ...
    if selected_city and selected_country:
        ...
        st.warning("Veuillez sélectionner une ville et un pays pour afficher les informations touristiques.")
