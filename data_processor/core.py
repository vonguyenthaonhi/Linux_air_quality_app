...
try:
    ...
    print(
        f"Erreur lors de la lecture des données qualité de l'air : {e}"
    )
    exit(1)
...
try:
    df_valid_cities = pd.read_csv(world_cities_path, encoding="utf8")
...
if "Coordinates" in data.columns:
    ...
    if coord_data.shape[1] == 2:
        ...
        print(
            "Erreur : format des coordonnées incorrect. Vérifiez le format."
        )
        exit(1)
else:
